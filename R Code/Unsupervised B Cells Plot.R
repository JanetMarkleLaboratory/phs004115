...
raw_counts <- LayerData(B_cells, assay = "RNA", layer = 'counts')
norm_counts <- LayerData(B_cells, assay = "RNA", layer = 'data')
ref <- celldex::HumanPrimaryCellAtlasData()
unique(ref$label.fine)
unique(ref$label.main)
ref <- ref[,grep('B_cell', ref$label.main)]
ref <- ref[,grepl('B_cell|B_cell:Naive|B_cell:Memory|B_cell:Plasma|B_cell:immature', ref$label.fine)]
...
DimPlot(B_cells_ann, reduction = 'umap', split.by = 'new.ident')
...
positive <- gs_list$gs_positive[c(1:5)]
negative <- gs_list$gs_negative[c(1:5)]
positive
negative
##############
# run ScType
...
seurat_package_v5 <- isFALSE('counts' %in% names(attributes(B_cells[["RNA"]])))


# extract scaled scRNA-seq matrix
scRNAseqData_scaled <- if (seurat_package_v5) as.matrix(B_cells[["RNA"]]$scale.data) else as.matrix(B_cells[["RNA"]]@scale.data)


cL_resutls <- do.call("rbind", lapply(unique(B_cells@meta.data$RNA_snn_res.0.65), function(cl){
  es.max.cl = sort(rowSums(es.max[ ,rownames(B_cells@meta.data[B_cells@meta.data$RNA_snn_res.0.65==cl, ])]), decreasing = !0)
  head(data.frame(cluster = cl, type = names(es.max.cl), scores = es.max.cl, ncells = sum(B_cells@meta.data$RNA_snn_res.0.65==cl)), 10)
}))
...
B_cells@meta.data$sctype_classification = ""
for(j in unique(sctype_scores$cluster)){
  cl_type = sctype_scores[sctype_scores$cluster==j,]; 
  B_cells@meta.data$sctype_classification[B_cells@meta.data$RNA_snn_res.0.65 == j] = as.character(cl_type$type[1])
}

DimPlot(B_cells, reduction = "umap", label = TRUE, repel = TRUE, group.by = 'sctype_classification', split.by = 'new.ident')        
table(B_cells$sctype_classification, B_cells$RNA_snn_res.0.65)
table(B_cells$sctype_classification, B_cells$new.ident)
...
celltypes_hn <- pbmc_small$groups
